...
Private fields live under Studio fieldset **Registration (private)**. They must never appear in public GROQ projections, pages, feeds, sitemaps, or client bundles.

### Registration & credentials

The product-ID webhook and `/admin/sessions` blast path are retired. Purchase → confirmation → 8-day credentials flow is documented in **`docs/workshop-registration-system.md`** (Payment Link metadata `workshop_slug` / `series_slug`, Sanity `registration` docs, cron).

Ops checklist and test cards live in that doc — do not reintroduce product-ID mapping here.
